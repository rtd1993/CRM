                ...
                content.innerHTML = data.tasks.map(task => `
                    <div class="client-task-item ${task.ricorrente ? 'ricorrente' : ''} ${task.scaduto ? 'scaduto' : ''}">
                        ...
                        <div class="task-actions">
                            ...
                            ${task.link_cartella ? `<a href="drive.php?path=${encodeURIComponent(task.link_cartella)}" class="btn btn-secondary btn-sm" target="_blank" title="Apri cartella cliente"><i class="fas fa-folder-open"></i></a>` : ''}
                        </div>
                    </div>
                `).join('');
