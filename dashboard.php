...
<div style="display: flex; justify-content: space-between; gap: 20px; flex-wrap: wrap;">
    ...
    <div style="flex: 2; min-width: 400px;">
        ...
        $stmt = $pdo->prepare("
            SELECT id, descrizione, scadenza
            FROM task
        ...
        <?php if (empty($tasks)): ?>
            <p>Nessun task in scadenza entro 30 giorni.</p>
        <?php else: ?>
            <table style="width: 100%; border-collapse: collapse;">
                <thead>
                    <tr style="background: #f5f5f5;">
                        <th style="border: 1px solid #ccc; padding: 6px;">Nome Task</th>
                ...
                ?>
                    <tr style="<?= $style ?>">
                        <td style="border: 1px solid #ccc; padding: 6px;">
                            <a href="task.php" style="text-decoration: underline; color: #007bff;">
                                <?= htmlspecialchars($t['descrizione']) ?>
                            </a>
                        </td>
                        <td style="border: 1px solid #ccc; padding: 6px;"><?= htmlspecialchars($t['scadenza']) ?></td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
            </table>
        ...
        <?php else: ?>
            <table style="width: 100%; border-collapse: collapse;">
                ...
                ?>
                    <tr style="<?= $scaduto ?>">
                        <td style="border: 1px solid #ccc; padding: 6px;">
                            <a href="clienti_scheda.php?id=<?= urlencode($doc['id']) ?>" style="text-decoration: underline; color: #007bff;">
